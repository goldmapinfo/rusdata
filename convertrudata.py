...
with open(input_file_path, 'r', encoding='utf-8') as infile, \
    ...
    for row in reader:
        try:
            # Удаляем строки, где в 16-м столбце содержится "Аннулированно"
            if len(row) > 15 and "Аннулирование" in row[15]:
                continue  # Пропускаем строку

            # Удаляем строки, где в 17-м столбце (индекс 16) есть любые данные
